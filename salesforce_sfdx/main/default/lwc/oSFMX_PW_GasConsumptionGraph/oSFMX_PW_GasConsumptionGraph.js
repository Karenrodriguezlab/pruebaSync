...
export default class oSFMXGasConsumptionGraph extends LightningElement {
  ...
  aplicarFiltro(){    
    ...
    if (this.filtros['valueDatestart']!=="Todos"){
      consumosTemp = consumosTemp.filter((item)=>{
        return item.date_start==this.filtros['valueDatestart'];
      });
    }
    if (this.filtros['valueDateend']!=="Todos"){
      consumosTemp = consumosTemp.filter((item)=>{
        return item.date_end==this.filtros['valueDateend'];
      });
    }
...
//Exportar Hacía CSV - INICIO
    downloadCSVFileJS() {
        ...
        this.csvString = '';
        let rowEnd = '\n';
        let rowData = new Set(['nombre_cliente','nombre_suministro','anio','mes',
        'consumo_facturado_gj','estado_desc','instal_suministro']);
        let csvHeader = new Set(
            ['Nombre Cliente','Nombre Suministro SAP','Año','Mes',
            'Consumo Facturado GJ','Estado','Instalación Suministro SAP']
        );
