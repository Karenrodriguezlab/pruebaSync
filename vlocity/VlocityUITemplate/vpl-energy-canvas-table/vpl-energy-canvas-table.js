        ...
        var isFound = true;
         if($scope.searchTerm) {
            ...
            _.some(row.states[0].fields, function(field){
               var fieldValue = getterFilter(rawData, field) !== null && typeof getterFilter(rawData, field) !== 'undefined' ? getterFilter(rawData, field).toString().toUpperCase() : '';
...
//sforce.console.openSubtab(result.id,'/apex/%vlocity_namespace%__ConsoleCards?id='+$scope.params.id+'&layout=Asset-Layout-View-All&inConsole=true', true, 'Subtab Name', null); 
window.parentIFrame.sendMessage({
                        message:'actionLauncher:windowopen',
                        action: {
                            type: 'Custom',
                            isCustomAction: true,
                            url: '/apex/%vlocity_namespace%__ConsoleCards?id='+$scope.params.id+'&layout=Asset-Layout-View-All',
       ...
       //window.open('/apex/%vlocity_namespace%__ConsoleCards?id='+$scope.params.id+'&layout=Asset-Layout-View-All')
    }
